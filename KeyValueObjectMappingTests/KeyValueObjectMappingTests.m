...
@property(nonatomic,strong) NSDictionary *plist, *json;
@end
...
- (void)setUp {  [super setUp];
  ...
  self.plist = [NSDictionary dictionaryWithContentsOfFile:pathToFile];
...
- (void) testValidPlistToPerson
{
  ...
  Person *person = [parser parseDictionary:self.plist];
  ...
  STAssertEqualObjects(person.nota, @10, @"Should be equals");
  ...
  STAssertEqualObjects(person.arrayPrimitive[0], @"hello", @"Should have hello on first position of array");
  STAssertEqualObjects(person.arrayPrimitive[1], @"mutchaco", @"Should have muthaco on first position of array");
  STAssertEqualObjects(person.arrayPrimitive[2], @1, @"Should have muthaco on first position of array");
  STAssertEqualObjects(person.arrayPrimitive[3], @3.1416, @"Should have muthaco on first position of array");
  configuration = nil;
}
...
-(void) testValidJsonToTweet {
  ...
  Tweet                    * tweet = [parser parseDictionary:self.json];
  ...
  STAssertTrue(tweet.retweetCount == 0, @"RetweetCount should be equals to 0, got %lu", tweet.retweetCount);
  STAssertFalse(tweet.favorited, @"favorited should be false");
  STAssertFalse(tweet.retweeted, @"retweeted should be false");
...
- (void) testValidJsonToArrayOfTweets{
  ...
  NSArray *arrayTweets = @[self.json, self.json, self.json];
  DCParserConfiguration *configuration = [DCParserConfiguration configuration];
...
- (void) testValidJsonToUserWithMultipleTweetsAsProperty{
  Class tweetClass = [Tweet class];
  NSMutableDictionary *userDictionary = [NSMutableDictionary dictionaryWithDictionary:self.json[@"user"]];

  NSArray *tweetsForUser = @[self.json, self.json];
  userDictionary[@"tweets"] = tweetsForUser;
...
- (void) testNullValuesPassed
{
  ...
  person = [parser parseDictionary:@{}];
  STAssertNil(person, @"Should be nill when class is nil");

  persons = [parser parseArray:@[]];
...
-(void) testShouldUseCustomInitializeForPropertyClasses {
  ...
  DCCustomInitializeBlock block = ^id(__weak Class classToGenerate, __weak NSDictionary *values, id parentObject){
    ...
    STAssertEqualObjects(values[@"name"], @"Diego Chohfi", @"Should have same user name");
...
-(void) testCustomMap {
  ...
  [userValues setValue:@[tweet1, tweet2] forKey:@"tweets"];
